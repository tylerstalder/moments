 ...
 * Author: Tom Carden
 * http://bl.ocks.org/600144
 */

 /*
 * Modifications:
 * Cleaned up a few semi-colons and variable definitions to pass jslint
 */

var Fly = function(map, bbox) {

var interval;
var po = org.polymaps;

function panMap(map, bbox) {
  ...
  var bl = map.locationPoint({ lat: bbox[0].lat, lon: bbox[0].lon }),
  tr = map.locationPoint({ lat: bbox[1].lat, lon: bbox[1].lon }),
  sizeActual = map.size(),
